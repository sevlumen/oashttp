...
func (r Result[T]) WriteHTTPWithFailureFormatter(w http.ResponseWriter, onError func(error), formatter core.FailureFormatter, failureContentType string) {
	...
	if status < 200 || status > 599 {
		...
		internalfailure.WriteResolved(w, formatter, failureContentType, core.Failure{Status: http.StatusInternalServerError, Code: "RESPONSE_SERIALIZATION_FAILED", Detail: "The response could not be serialized"}, onError)
		return
	}
