...
func benchmarkHandler(b *testing.B) http.Handler {
	app := oashttp.New(oashttp.Config{Info: oashttp.Info{Title: "Bench", Version: "1.0.0"}})
	group := app.Group("")
	oashttp.MapGet(group, "/users/{id:uuid}", func(_ context.Context, input benchmarkGetInput) oashttp.Result[benchmarkOutput] {
		return oashttp.OK(benchmarkOutput{ID: input.ID, Name: "Alice"})
	}).WithOperationID("getBenchUser").Produces(http.StatusOK)
	oashttp.MapPut(group, "/users/{id:uuid}", func(_ context.Context, input benchmarkInput) oashttp.Result[benchmarkOutput] {
		return oashttp.OK(benchmarkOutput{ID: input.ID, Name: input.Body.Name})
