...
func compileStruct(t reflect.Type, prefix []int, out *[]fieldPlan, stack map[reflect.Type]bool) error {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if nested.Kind() == reflect.Struct && nested.PkgPath() != "time" && !(nested.PkgPath() == "github.com/oashttp/oashttp" && nested.Name() == "UUID") && !nested.Implements(textUnmarshalerType) && !reflect.PointerTo(nested).Implements(textUnmarshalerType) {
